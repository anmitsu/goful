...
func config(g *goful.Goful) {
	...
	g.MergeExtmap(widget.Extmap{
		"C-m": { // associates by file types with the enter key event
			...
			".tgz":   func() { g.ShellMode("tar xvfz %f -C %D") },
			".bz2":   func() { g.ShellMode("tar xvfj %f -C %D") },
			".tar":   func() { g.ShellMode("tar xvf %f -C %D") },
			".rar":   func() { g.ShellMode("unrar x %f -C %D") },
			".java":  func() { g.ShellMode("javac %f") },
			".class": func() { g.ShellMode("java %x") },
			".jar":   func() { g.ShellMode("java -jar %f") },
			".py":    func() { g.ShellMode("python %f") },
			".go":    func() { g.ShellMode("go run %f") },
		},
		"v": {
...
func filerKeymap(g *goful.Goful) widget.Keymap {
	return widget.Keymap{
		...
		"C-f":       func() { g.Workspace().MoveCursor(1) },
		"C-b":       func() { g.Workspace().MoveCursor(-1) },
		"right":     func() { g.Workspace().MoveCursor(1) },
		"left":      func() { g.Workspace().MoveCursor(-1) },
